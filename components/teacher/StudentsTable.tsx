...
export default function StudentsTable({
  ...
  data: { student: string; gradeData: GradeData[]; problem: Problem }[];
}) {
  return (
    <Table>
      <TableHeader>
        ...
        {data.map(({ gradeData, student, problem }, i) => (
          <TableRow key={i}>
                ...
                {isProblemEmpty(problem) ? null : (
                  <Dialog>
                    <DialogTrigger asChild>
                      <Badge className="bg-muted cursor-pointer">
                        {problem.problem}
                      </Badge>
                    </DialogTrigger>
                    <DialogContent>
                      ...
                      <div className="grid gap-2 py-4">
                        <p>
                          <span className="font-bold">Opis problemu:</span>
                          <p />
                          {problem["problemDesc"]}
                        </p>
                      </div>
                      {Array.from({ length: problem.problemCause.length }).map(
                        (e, idx) => (
                          <li
                            key={idx}
            ...
            <TableCell className="text-center max-w-8">
              {getAvg(gradeData).toPrecision(3)}
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
